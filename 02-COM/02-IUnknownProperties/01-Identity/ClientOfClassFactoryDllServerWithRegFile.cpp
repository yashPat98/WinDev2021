...
IUnknown *gpIUnknowFromISum = NULL;
IUnknown *gpIUnknowFromISubtract = NULL;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(iMsg)
    {
        case WM_CREATE:
            ...
            //request service from dll
            gpISubtract->SubtractionOfTwoIntegers(num1, num2, &subtract);

            //display result 
            wsprintf(str, TEXT("Subtraction of %d and %d = %d"), num1, num2, subtract);
            MessageBox(hwnd, str, TEXT("Result"), MB_OK);

            gpISum->QueryInterface(IID_IUnknown, (void**)&gpIUnknowFromISum);
            gpISum->QueryInterface(IID_IUnknown, (void**)&gpIUnknowFromISubtract);

            if(gpIUnknowFromISum == gpIUnknowFromISubtract)
            {
                MessageBox(hwnd, TEXT("identity"), TEXT("same"), MB_OK);
            }

            gpIUnknowFromISum->Release();
            gpIUnknowFromISum = NULL;
